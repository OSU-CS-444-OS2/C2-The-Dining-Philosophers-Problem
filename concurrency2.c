...
which means philosophers pick forks when their neighbors are not eating, return both forks when finish eating.
...
pthread_mutex_t mutex=PTHREAD_MUTEX_INITIALIZER; ////initialize mutex,only one philosopher can get forks or put forks at one moment
sem_t S[N];
...
int main(int argc, char *argv[]){
	...
	for(i=0;i<philos_num;i++){
		...
		//create threads and philosophers start thinking
        printf("philosopher %s is thinking now !\n",philos_names[i]);
	}
	for(i=0;i<philos_num;i++){
